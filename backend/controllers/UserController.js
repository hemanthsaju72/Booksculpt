...
exports.signup = async (req, res) => {
  try {
    ...
    const user = new User({ name, email, password, recheckPassword: password });
    ...
    console.log(cart);
    const token = jwt.sign({ id }, "nikkky", { expiresIn: "9000000" });
    const cookieOption = {
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json({ message: "Email or password is incorrect {email}" });
    }

    if (password !== user.recheckPassword) {
      return res
        .status(401)
        .json({ message: "Email or password is incorrect" });
    }
    let id = user.id;
    const token = jwt.sign({ id }, "test", { expiresIn: "90000000" });
...
exports.makeAdmin = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ updatedUser, message: "User made admin successfully" });
  } catch (error) {
...
exports.removeAdmin = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ updatedUser, message: "Admin privileges removed successfully" });
  } catch (error) {
...
exports.unblockUser = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ updatedUser, message: "User unblocked successfully" });
  } catch (error) {
